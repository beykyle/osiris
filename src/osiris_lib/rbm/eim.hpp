...
/// @brief  Handles the affine-decomposition of a potential where energy and reduced mass are treated as
/// parameters. By default, energy is the first and reduced mass the second elements of the parameter
/// arrays passed in to an instance of this class; the rest are passed into the call to potential
...
public:
  ...
  EnergizedEIMInteractionSpace(
      ...
      const std::vector<potential_t>& potentials,
      array3d_t Ainv_matrices, 
      array1d_t r_matches)
      : mesh_size(mesh_size), 
        nbasis(nbasis), 
        lmax(lmax),
        r_matches(r_matches), 
        Ainv_matrices(Ainv_matrices),
