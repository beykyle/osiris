...
TEST_CASE("BSP with depth < dimensions") {
  ...
  auto bsp = BinarySPTree<int>(2, {-1., -1, -1}, {1, 1, 1}, {0, 1, 2, 3});

  // x<0, y<0
  REQUIRE(bsp[{-1. / 2, -1. / 2, 1. / 2}] == 0);
  REQUIRE(bsp[{-1. / 2, -1. / 2, -1. / 2}] == 0);

  // x<0, y>0
  REQUIRE(bsp[{-1. / 2, 1. / 2, 1. / 2}] == 1);
  REQUIRE(bsp[{-1. / 2, 1. / 2, -1. / 2}] == 1);

  // x>0, y<0
  REQUIRE(bsp[{1. / 2, -1. / 2, 1. / 2}] == 2);
  REQUIRE(bsp[{1. / 2, -1. / 2, -1. / 2}] == 2);

  // x>0, y>0
  REQUIRE(bsp[{1. / 2, 1. / 2, 1. / 2}] == 3);
  REQUIRE(bsp[{1. / 2, 1. / 2, -1. / 2}] == 3);
}
...
TEST_CASE("BSP with depth > dimensions") {
  ...
  //     0    1/2    1
  //
  auto bsp = BinarySPTree<int>(3, {0, 0}, {1, 1}, {0, 1, 2, 3, 4, 5, 6, 7});
...
TEST_CASE("BSP with fixed dim") {
  ...
  //     0    1/2    1
  //
  auto bsp = BinarySPTree<int, std::array<double, 2>>(
      3, {0, 0}, {1, 1}, {0, 1, 2, 3, 4, 5, 6, 7});
...
TEST_CASE("BSP with custom point") {
  ...
  auto bsp = BinarySPTree<int, Point>(
      3, Point{.x = 0, .y = 0}, Point{.x = 1, .y = 1},
      {0, 1, 2, 3, 4, 5, 6, 7});
